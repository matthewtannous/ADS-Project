...
private:
  ...
  public:
     ...
     * bool: true if user1 sent the message, false if user2 sent the message
     * (bool uses less memory)
     */
...
public:
  ...
  // Cross-user actions: enforce authorization (self or friend) and emit
  // notifications Like another user's post; notifies the owner
  ...
  bool sendMessage(const string &sender, const string &recipient,
                   const string &text);

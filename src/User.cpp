...
void User::addPost(Post p) { postsList.push_back(p); }
...
bool User::removePostAt(int index) {
  if (index < 0 || static_cast<size_t>(index) >= postsList.size())
    return false;
...
void User::displayFriends() {
  ...
  if (!closeFriendsList.empty()) {
    cout << "Close friends:\n";
    for (const string &f : closeFriendsList)
      cout << "  * " << f << "\n";
  }
}
...
void User::removeCloseFriend(const string &u) { closeFriendsList.erase(u); }

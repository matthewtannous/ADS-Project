...
bool Users::sendMessage(const string &sender, const string &recipient,
                        const string &text) {
...
Users::Conversation *Users::findOrCreateConversation(const string &a,
                                                     const string &b) {
  Conversation *c = findConversation(a, b);
  if (c != nullptr)
    return c;
...
void Users::Conversation::addMessage(string sender, string message) {
  // Add message at the beginning so users see newest messages first and for
  // faster insertion

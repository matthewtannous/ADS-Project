...
Post::Post(string username, string description)
...
void Post::addComment(const string &commenter, const string &text,
...
string Post::getDescription() const { return description; }

int Post::getPostId() const { return postId; }

string Post::getUsername() const { return username; }

int Post::getLikesCount() const { return static_cast<int>(likedBy.size()); }

int Post::getCommentCount() const {
  return static_cast<int>(commentsList.size());
}

void Post::like(const string &liker) { likedBy.insert(liker); }
